  <script>
	...
	// GST Calculator variables and function
	let price = 0, gst = 0, gstAmount = 0, finalPrice = 0, errorMessage = '';
	function calculateGST() {
	  if (price < 0 || gst < 0) {
		errorMessage = 'Price and GST cannot be negative.';
		gstAmount = 0;
		finalPrice = 0;
	  } else {
  ...
  {#if selectedCalculator === 'gst'}
	<div class="calculator p-6 rounded-lg shadow-lg max-w-md mx-auto mt-10 mb-10">
	  ...
	  <label class="block mb-2">
		Price of Product:
		<input type="number" bind:value={price} class="border border-gray-300 rounded w-full py-2 px-3 mt-1" placeholder="Enter Price of Product" />
	  </label>
	  <label class="block mb-2">
		GST Tax (%):
		<input type="number" bind:value={gst} class="border border-gray-300 rounded w-full py-2 px-3 mt-1" placeholder="Enter % of GST" />
	  </label>
	  <button on:click={calculateGST} class="bg-blue-500 text-white py-2 px-4 rounded w-full font-bold">
		Calculate Price with GST
	  </button>
	  {#if errorMessage}
		<div class="text-red-500 text-sm mt-4">{errorMessage}</div>
	  {/if}
	  <p class="mt-4 text-lg font-semibold">GST Amount: ₹{gstAmount.toFixed(2)}</p>
